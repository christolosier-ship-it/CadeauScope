...
export async function importJson(data){ if(data?.app!==APP.name || data.schemaVersion!==1) throw new Error('Sauvegarde CadeauScope incompatible.'); await clearDb(); const ids={ people:new Set((data.people||[]).map(x=>x.id)), occasions:new Set((data.occasions||[]).map(x=>x.id))}; for(const p of data.people||[]) await repositories.people.put({...p, photoId:''}); for(const c of data.categories||[]) await repositories.categories.put(c); for(const o of data.occasions||[]) await repositories.occasions.put(o); for(const i of data.ideas||[]) await repositories.ideas.put({...i, photoId:'', personId:ids.people.has(i.personId)?i.personId:'', occasionId:ids.occasions.has(i.occasionId)?i.occasionId:''}); for(const h of data.history||[]) await repositories.history.put(h); if(data.settings) await repositories.settings.put({...data.settings, exportPhotos:false}); await seedDefaults(); }
