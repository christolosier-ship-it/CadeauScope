...
export async function savePerson(data){ const errors=validatePerson(data); if(hasErrors(errors)) throw Object.assign(new Error('Personne invalide'),{errors}); const existing=data.id ? await getPerson(data.id) : null; const item=createPerson({...existing,...data,createdAt:existing?.createdAt}); await repositories.people.put(item); return item; }
export async function archivePerson(id, archived=true){ const p=await getPerson(id); if(!p) return null; const next={...p,archived,updatedAt:nowIso()}; await repositories.people.put(next); return next; }
...
export async function deletePerson(id){ if(!(await canDeletePerson(id))) throw new Error('Suppression bloquée: cette personne a encore des idées ou de l’historique.'); await repositories.people.delete(id); }
