...
async function start(){ await seedDefaults(); const settings=await getSettings(); applyTheme(settings); if('serviceWorker' in navigator){ navigator.serviceWorker.register('./service-worker.js').catch(()=>{}); }
